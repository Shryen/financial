...
export default function Index({ transactions }) {
    ...
    const total = transaction.reduce((acc, item) => acc + item.price, 0);

    const add = useAdd({
        array: transaction,
        setArray: setTransaction,
    ...
    function deleteRow(id) {
        Swal.fire({
            title: "Biztos, hogy törli a kiválasztott tranzakciót?",
            icon: "warning",
    ...
    function sendData(e) {
        e.preventDefault();
        const dataToSend = transaction.map((f) => {
            return {
                name: f.name,
                price: f.price,
            };
        ...
        router.post("/transaction", dataToSend);
        router.on("success", () =>
            Swal.fire({
                title: "Sikeres",
                icon: "success",
        ...
        <Layout>
            ...
            {transaction.length !== 0 ? (
                        ...
                        <div key={f.id}>
                            ...
                            >
                                <p
                                    ...
                                    onClick={() => {
                                        window.location.href = `/transactions/${f.id}`;
                                    }}
                                >
                                    {f.name}
                                </p>
                ...
                <p className="text-red-500 font-bold">
                    Nincsenek tranzakciók, adj hozzá valamit!
                </p>
            )}
