...
export default function Index({ invoices }) {
    ...
    const total = invoice.reduce((acc, item) => acc + item.price, 0);
    ...
    function deleteRow(id) {
        Swal.fire({
            title: "Biztos, hogy törli a kiválasztott számlákat?",
            icon: "warning",
    ...
    function sendData(e) {
        e.preventDefault();
        const dataToSend = invoice.map((f) => {
            return {
                name: f.name,
                price: f.price,
            };
        ...
        router.post("/invoices", dataToSend);
        router.on("success", () =>
            Swal.fire({
                title: "Sikeres",
                icon: "success",
        ...
        <Layout>
            ...
            {invoice.length !== 0 ? (
                <div>
                    ...
                    >
                        <p className="font-bold uppercase text-lg">Számla</p>
                        ...
                        <div key={f.id}>
                            ...
                            >
                                <p
                                    ...
                                    onClick={() => {
                                        window.location.href = `/invoices/${f.id}`;
                                    }}
                                >
                                    {f.name}
                                </p>
