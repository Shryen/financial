...
export default function Show({ invoices }) {
    return (
        <Layout>
            <div className="text-lg">
                ...
                <div className="mt-2 leading-6 grid grid-cols-2 place-content-evenly w-2/3 mx-auto">
                    <p className="w-full">Számla neve: </p>
                    <p className=" text-right">{invoices.name}</p>
                    <p className="w-full">Számla ára: </p>
                    <p className="w-full text-right">{invoices.price}</p>
                </div>
                <Button
                    onClick={() =>
                        (window.location.href = `/invoices/${invoices.id}/edit`)
                    }
